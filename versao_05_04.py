...
def tamanho_quadrantes(arq_instancia):
    ...
    # Analisando se o número de centros possui raíz quadrada inteira
    ...
    # Analisando qual o múltiplo do número de centros
    multiplo = 1
    for i in range(2, 6):
        if qtde_centros % i == 0:
            multiplo = i
    ...
    # Dimensões dos quadrantes em tabela / quadrados
    elif possui_raiz:
        tam_quad_x = tam_x / resultado_raiz
        tam_quad_y = tam_y / resultado_raiz

    # Dimensões dos quadrantes em tabela / retângulos
    else:
        ...
        while True:
            ...
            if qtde_pontos_quad >= lig_min:
                ...
                if qtde_livre > 0:  # irá pegar os pontos livres
                    coordenadas_por_quadrante[i].append(nao_usados[-1])
                    ...
                    for c in range(1, qtde_centros + 1):  # loop para descobrir o quadrante com mais pontos
                        ...
                        if tam_quad > maior_quad:
                            maior_quad = tam_quad
                            n_quad = c
                    coordenadas_por_quadrante[i].append(coordenadas_por_quadrante[n_quad][-1])
                    sorted(coordenadas_por_quadrante[i])
                    del coordenadas_por_quadrante[n_quad][-1]
        ...
        if qtde_livre == 0:
            ...
            for c in range(1, qtde_centros + 1):  # loop para descobrir o quadrante com menos pontos
                ...
                if tam_quad < menor_quad:
                    menor_quad = tam_quad
                    n_quad = c
            coordenadas_por_quadrante[n_quad].append(nao_usados[-1])
