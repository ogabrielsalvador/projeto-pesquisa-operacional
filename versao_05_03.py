...
def tamanho_quadrantes(arq_instancia):
    ...
    if qtde_centros % 2 == 0:       # número de centros é par
        tam_quad_x = tam_x / 2
        tam_quad_y = tam_y / (qtde_centros / 2)
    else:                           # número de centros é ímpar
        tam_quad_x = tam_x
...
def dict_quadrantes_coordenadas_provisorio(arq_instancia):
    ...
    # ADICIONANDO OS PONTOS DAS INSTÂNCIAS COM QUADRANTES PAR
    if qtde_centros % 2 == 0:

        # CRIAÇÃO DE UMA LISTA REFERENTES A LINHA.
        linha = list()

        for n in range(1, qtde_centros + 1):  # Irá pegar todos os pontos da linha.
            ...
            inicio_x = min_x
            final_x = min_x + tam_quad_x

            for c in range(len(linha)):
                if n % 2 == 1:      # se for um quadrante ímpar, fará parte da primeira coluna
                    x = linha[c][0]
                    if inicio_x <= x <= final_x:
                        listados.append(linha[c])

                        # Apagando os pontos usados da lista 'linha'
                        nao_usados = list(filter((linha[c]).__ne__, nao_usados))

                else:               # se for um quadrante par, fará parte da segunda coluna
                    listados.append(linha[c])
    ...
    # ADICIONANDO OS PONTOS DAS INSTÂNCIAS COM QUADRANTES ÍMPAR
    else:
    ...
    for i in range(1, qtde_centros + 1):
        ...
        while True:
            ...
            if qtde_pontos_quad <= lig_max:
                ...
                nao_usados.append(coordenadas_por_quadrante[i][0])
                sorted(nao_usados)
                del coordenadas_por_quadrante[i][0]
        ...
        while True:
            ...
            if qtde_pontos_quad >= lig_min:
                ...
                if qtde_livre > 0:  # irá pegar os pontos livres
                    coordenadas_por_quadrante[i].append(nao_usados[0])
                    ...
                    for c in range(1, qtde_centros + 1):  # loop para descobrir o quadrante com mais pontos
                        ...
                        if tam_quad > maior_quad:
                            maior_quad = tam_quad
                            n_quad = c
                    coordenadas_por_quadrante[i].append(coordenadas_por_quadrante[n_quad][0])
                    sorted(coordenadas_por_quadrante[i])
                    del coordenadas_por_quadrante[n_quad][0]
        ...
        if qtde_livre == 0:
            ...
            for c in range(1, qtde_centros + 1):  # loop para descobrir o quadrante com menos pontos
                ...
                if tam_quad < menor_quad:
                    menor_quad = tam_quad
                    n_quad = c
            coordenadas_por_quadrante[n_quad].append(nao_usados[0])
