...
def dict_quadrantes_coordenadas(arq_instancia):
    ...
    # fica em loop até o min e o max estar certo
    while True:
...
def correcao(arq_instancia, arq_solucao):
    ...
    # MEU RESULTADO
    solucao = jujuba(arq_instancia, arq_solucao)[0]
    qtde_pontos_sol = 0
    """maior_dist_sol = jujuba(arq_instancia, arq_solucao)[1]"""

    # INÍCIO DA CORREÇÃO
    print(f'\nCorreção da instância {arq_instancia[16:18]}\n')

    # Corrigindo o número de centros
    qtde_centros_sol = len(solucao)
    if qtde_centros_sol == qtde_centros_gab:
        print(f'CORRETO! O número de centros da instância é: {qtde_centros_sol}.\n')
    else:
        print(f'ERRO! O número de centros deve ser: {qtde_centros_gab}.\n'
              f'Mas a solução possui {qtde_centros_sol}.\n')
    ...
    for i in range(qtde_centros_sol):
        ...
        if lig_min_gab <= qtde_pontos_quad <= lig_max_gab:
            print(f'CORRETO! Cada quadrante deve ter entre {lig_min_gab} e {lig_max_gab} pontos.\n'
                  f'O quadrante {i} possui {qtde_pontos_quad}\n')
        elif lig_max_gab < qtde_pontos_quad:
            print(f'ERRO! O número máximo de pontos por quadrante deve ser: {lig_max_gab}\n'
                  f'Mas o quadrante {i} possui {qtde_pontos_quad}\n')
        else:
            print(f'ERRO! O número mínimo de pontos por quadrante deve ser: {lig_min_gab}\n'
                  f'Mas o quadrante {i} possui {qtde_pontos_quad}\n')

    # Corrigindo o número de pontos
    if qtde_pontos_sol == qtde_pontos_gab:
        print(f'CORRETO! O número de pontos da instância é: {qtde_pontos_sol}.\n')
    else:
        print(f'ERRO! O número de pontos deve ser: {qtde_pontos_sol}.\n'
              f'Mas a solução possui {qtde_pontos_sol}.\n')

    # FIM DA CORREÇÃO
    print(f'{"=" * 60}')

"""
    # Verificando o GAP da distancia
    gap = int(((maior_dist_sol / maior_dist_gab) - 1) * 100)
    if gap <= 20:
        print(f'PERFEITO! O seu GAP foi de {gap:.0f}% em comparação ao valor de referência\n')
    elif gap <= 50:
        print(f'BOM! O seu GAP foi de {gap}% em comparação ao valor de referência\n')
    elif gap <= 100:
        print(f'QUASE! O seu GAP foi de {gap}% em comparação ao valor de referência\n')
    elif gap <= 200:
        print(f'RUIM! O seu GAP foi de {gap}% em comparação ao valor de referência\n')
    else:
        print(f'PÉSSIMO! O seu GAP foi de {gap}% em comparação ao valor de referência\n')
"""
